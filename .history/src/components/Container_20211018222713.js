...
const Container = () => {
  ...
  const [imagee, setImagee] = useState('assets/planet-earth-internal.svg')
  ...
  function buttonChange(o) {
    ...
    if (selectBox === 1) {
      ...
      setImagee(data.filter(d => d.name === planet)[0].planet);
    }

    if (selectBox === 2) {
      setText(data.filter(d => d.name === planet)[0].structure);
      setImagee(data.filter(d => d.name === planet)[0].internal);

    }

    if (selectBox === 3) {
      setText(data.filter(d => d.name === planet)[0].geology);
      setImagee(data.filter(d => d.name === planet)[0].geology);
